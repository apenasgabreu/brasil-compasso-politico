...
function Portraits({ portraits, compact = false }: { portraits: ReturnType<typeof getProgramMeta>["portraits"]; compact?: boolean }) {
  if (!portraits.length) return null;
  return <div className={`portrait-stack ${compact ? "portrait-stack-compact" : ""}`} aria-label={`Retrato de ${portraits.map((portrait) => portrait.name).join(" e ")}`}>{portraits.map((portrait) => <a href={portrait.source} target="_blank" rel="noreferrer" title={`Fonte da foto: ${portrait.credit}`} key={portrait.name}><img src={portrait.image} alt={`Retrato de ${portrait.name}`} /></a>)}</div>;
}
...
export default function Home() {
  ...
  const renderMethod = () => <div className="method-shell"><SectionTitle eyebrow="Transparência metodológica" title="O resultado é reproduzível, e seus limites são visíveis." text="A ferramenta mede concordância com afirmações do questionário; não mede a qualidade de candidaturas nem substitui a decisão de voto." />
    ...
    <section className="programs-section"><div className="chart-heading"><div><p className="eyebrow">Base documental</p><h3>Programas analisados</h3></div><Pill>{programs.length} documentos</Pill></div><div className="program-catalog">{programs.map((program) => { const meta = getProgramMeta(program.program); return <a key={program.program} href={meta.url} target="_blank" rel="noreferrer"><Portraits portraits={meta.portraits} compact /><span>{meta.name}</span><small>{meta.document}</small><ExternalLink size={14} /></a>; })}</div></section><div className="stage-actions"><Button variant="outline" className="soft-button" onClick={() => navigate("intro")}><ArrowLeft size={16} /> Voltar</Button><Button className="primary-cta" onClick={() => navigate("weights")}>Fazer o questionário <ArrowRight size={17} /></Button></div></div>;
