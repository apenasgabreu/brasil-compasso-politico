...
import { axisOrder, contextualSourcePolicy, getProgramMeta, matrixIntegrity, methodologyChangelog, programs, questions, responseOptions, type Answer, type Axis } from "@/data/compassData";
...
export default function Home() {
  ...
  const recoverVault = async () => {
    try {
      ...
      const hydrated = payload.ranking.map((snapshotResult) => {
        const program = programs.find((item) => item.program === snapshotResult.program);
        if (!program) throw new Error("Este resultado usa uma matriz documental que não está disponível nesta versão.");
        return { program, score: snapshotResult.score, coverage: snapshotResult.coverage, economic: snapshotResult.economic, social: snapshotResult.social, axes: snapshotResult.axes } as ProgramScore;
      });
